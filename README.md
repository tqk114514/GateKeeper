...
### IP 识别机制
由于部署在 cloudflared 隧道后，TCP 层的源 IP 始终为 127.0.0.1。因此，GateKeeper 采用“严格请求头探测”机制：
- 网关会预读每个连接的前 16KB 数据（防止利用超大 Header 绕过探测）。
- 仅当连接来自 127.0.0.1 时，才会解析并信任 cf-connecting-ip 或 x-forwarded-for 标头。
- **强制约束**：若连接来自 127.0.0.1 但未探测到有效的真实 IP 标头，网关会将其视为非法请求并立即强制关闭连接。这防止了攻击者利用“回退到 127.0.0.1”引发对全站用户的速率限制攻击 (DoS)。
